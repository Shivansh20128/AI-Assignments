...
def unform_cost_search_algo(road_map, source, destination):
    ...
    while priority_queue:
        ...
        for connected_city in road_map[current_city]:
            better_path = shortest_distance_paths[current_city] + road_map[current_city][connected_city]['distance'] + admissible_heuristic(current_city,road_map)
            if better_path < shortest_distance_paths[connected_city]:
                shortest_distance_paths[connected_city] = better_path - admissible_heuristic(current_city,road_map)
                heapq.heappush(priority_queue, (better_path, connected_city))
...
def admissible_heuristic(city, road_map):
    connected_cities = list(road_map.neighbors(city))
    
    if not connected_cities:
        return 0
    
    min_distance = min(road_map[city][neighbor_cities]['distance'] for neighbor_cities in connected_cities)
    return -1
